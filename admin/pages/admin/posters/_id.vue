      ...
      <div class="poster">
        <div class="profile">
          ...
          <div class="art_profile flex">
            ...
            <el-row>
              ...
              <el-col>{{ poster?.user?.profile?.name }}</el-col>
            </el-row>
            <el-row>
              <el-col>Surname</el-col>
              <el-col>{{ poster?.user?.profile?.surname }}</el-col>
            </el-row>
            <el-row>
              <el-col>Patronymic</el-col>
              <el-col>{{ poster?.user?.profile?.patronymic }}</el-col>
            </el-row>
          </div>
        </div>
        <div class="poster_item">
          ...
          <el-row>
            ...
            <span v-html="poster.description"></span>
          </el-row>
          <el-row>
            <span>Images: </span>
            <div class="images_array flex gap-2.5">
              <div class="image_item" v-for="image in poster.images">
                <el-image
        ...
        <div class="is_active_poster">
          <form @submit.prevent="update">
            <div class="approve">
              <div>Approve?</div>
              <label class="switch">
                <input type="checkbox" v-if="poster.status_id === 1" checked v-model="form.checked">
...
export default {
  ...
  methods: {
    ...
    async update() {
      try {
        await this.$axios.patch('/admin/poster/approve/' + this.$route.params.id, this.form, {user_id: 1});
      } catch(e) {
        return;
      }
    }
