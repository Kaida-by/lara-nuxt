        ...
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <nuxt-link :to="'/admin/organizations/' + organizations[scope.$index].id">
                <el-button
                  size="mini"
              ...
              <el-button
                size="mini"
                type="danger" @click="deleteVacancy(organizations[scope.$index].id)">Delete</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="btns my-5">
  ...
  methods: {
    async searching() {
      if (this.searchInput.length > 2) {
        await this.$axios.get('/admin/organization/search', {
          params: {
        ...
        await this.$axios.get('/admin/organizations?page=' + this.page)
          .then((res) => {
    ...
    async fetchData() {
      await this.$axios.get(
          '/admin/organizations?page=' + this.page +
    ...
    async fetchDataCategories() {
      await this.$axios.get('/admin/organization-categories?categoryId=' + 2)
        .then((res) => {
    ...
    deleteVacancy(id) {
      if (confirm("Do you really want to delete this Organization?")) {
        try {
          this.$axios.delete('/admin/organization/delete/' + id).then(response => {
            this.fetchData();
          });
        } catch(e) {
          return;
    ...
    async countAllVacancies() {
      await this.$axios.get('/admin/count-organizations/')
          .then((res) => {
